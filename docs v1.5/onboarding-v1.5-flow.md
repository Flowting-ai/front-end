...
- **A1 — done, all 5 screens:**
  ...
  - Screen 3 (`src/app/(onboarding)/onboarding/profile/page.tsx`, nodes 11:459 / 55:2680) — first/last name + role dropdown + tone dropdown (reinstated 2026-08-29). Role and tone are held in `workspace-onboarding-context` and deliberately *not* PATCHed to the backend from this screen — see the "Fixed" entry below.
  - Screen 4 (`src/app/(onboarding)/onboarding/invite/page.tsx`, node 27:1353) — invite emails; this is also where `onboarding_completed` is set (mirrors the old flow's invite step) since screen 5 isn't a distinct route, and where screen 3's role/tone are finally sent (see below).
  ...
  - **Real bug found and fixed (2026-08-30): the invite screen (step 4) was unreachable whenever a user picked a Role and/or Tone on the profile screen.** The backend's `update_onboarding` (`services/users/repository.py`) silently sets `onboarding_completed=True` the instant `user_role` + `ai_tone` + `role_fit` are *all* set on the account — independent of which step set them, and with no way for the frontend to opt out. The profile screen used to PATCH `user_role`/`ai_tone` immediately on Next; since `role_fit` is already set by the workspace screen (step 2), filling in Role and/or Tone on the profile screen completed onboarding right there. The proxy's already-onboarded re-entry gate then bounced the subsequent `/onboarding/invite` navigation straight to `/`, landing on `/chat` and skipping the invite screen entirely. Fixed on the frontend only (no backend change): the profile screen (`profile/page.tsx`) no longer sends `user_role`/`ai_tone` — it only saves first/last name and keeps role/tone in `workspace-onboarding-context`. The invite screen's `finish()` (`invite/page.tsx`) now sends `user_role`/`ai_tone` together with `onboarding_completed: true` in the same PATCH, so all three gating fields land at the same moment as the explicit completion flag — the auto-complete side effect can no longer fire early. Both `profile/page.tsx` already called `refreshUser()` before navigating (added for a related, earlier stale-auth-state bug) — that call stays as a defensive backstop for a returning user who somehow already has `user_role`/`ai_tone` set from the old flow, but it does not by itself prevent this bug, since the proxy (not just the client's cached auth state) is what gates re-entry into `/onboarding/*`.
