  ...
  ({ statusFilter, onStatusFilterChange, children, className, ...rest }, ref) => {
    ...
    return (
      <Card
        ...
        {...rest}
      >
        <div className="flex flex-col gap-[10px] rounded-[14px] bg-white px-4 pt-[14px] pb-0">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className="cursor-pointer inline-flex items-center rounded-[8px] text-sm font-medium text-[var(--colors-gray-700,#374151)] hover:text-black"
                  style={{
                    minWidth: '128.25px',
                    height: '36px',
                    minHeight: '36px',
              ...
              <DropdownMenuContent align="end" className="w-[200px] rounded-[8px] bg-white p-1 border-0 shadow-lg">
                {STATUS_OPTIONS.map((option) => (
                  <DropdownMenuItem
                    ...
                    className="h-8 min-h-[32px] gap-2 rounded-[6px] px-2 py-[5.5px] text-[#111827] hover:bg-[#E5E5E5] hover:text-black focus:bg-[#E5E5E5] focus:text-black cursor-pointer"
                  >
                    {option.label}
                  </DropdownMenuItem>
                ))}
