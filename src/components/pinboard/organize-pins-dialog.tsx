...
export function OrganizePinsDialog({
  ...
  const handlePinUpdate = (updatedPin: PinType) => {
    if (initialPins.length > 0) {
      ...
      if (existingPin) {
        // Update existing pin
        onPinsUpdate(
          initialPins.map((p) => (p.id === updatedPin.id ? updatedPin : p))
        );
      } else {
  ...
  const handleBulkDelete = () => {
    if (onPinsUpdate) {
      const updatedPins = pinsToDisplay.filter(
        (p) => !selectedPinIds.includes(p.id)
      );
      onPinsUpdate(updatedPins);
    }
    setSelectedPinIds([]);
    ...
    if (onPinsUpdate) {
      const updatedPins = pinsToDisplay.map((pin) =>
        selectedPinIds.includes(pin.id)
          ? {
              ...pin,
              folderId: folderId === "unorganized" ? undefined : folderId,
            }
          : pin
      );
      onPinsUpdate(updatedPins);
  ...
  const confirmDeletePins = () => {
    if (onPinsUpdate) {
      const updatedPins = initialPins.filter(
        (pin) => !selectedPinIds.includes(pin.id)
      );
      onPinsUpdate(updatedPins);
    }
    setSelectedPinIds([]);
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        ...
        >
          <div className="organize-dialog-inner relative flex flex-col h-full overflow-hidden bg-white">
            <DialogHeader
              className="bg-white text-[#171717] space-y-0 flex-shrink-0"
              style={{
                paddingTop: "15px",
                paddingLeft: "24px",
                paddingRight: "24px",
            ...
            >
              <div className="flex items-center justify-between">
                <DialogTitle className="py-0 text-left text-[#171717] font-semibold text-base sm:text-lg">
                  Organize Pins
                </DialogTitle >
                <Button
                  variant="ghost"
              ...
              <div className="flex items-center gap-2 py-0">
                <div className="relative flex-1 max-w-[380px]">
                  ...
                  <input
                    type="text"
                    placeholder={
                      isMoveMode ? "Search folders..." : "Search pins..."
                    }
                    ...
                    style={{
                      ...
                      paddingLeft: "40px",
                      paddingRight: (
                        isMoveMode ? searchFolderQuery : searchQuery
                      )
                        ? "60px"
                        : "20px",
                      gap: "8px",
                    ...
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2 text-xs text-[#666666]">
                      {
                        folders.filter((folder) =>
                          folder.name
                            .toLowerCase()
                            .includes(searchFolderQuery.toLowerCase())
                        ).length
                      }{" "}
                      results
                    </div>
            ...
            <div
              className="flex-1 flex overflow-hidden px-6 min-h-0"
              style={{
                gap: "12px",
                width: "100%",
                backgroundColor: "#FFFFFF",
            ...
            >
              ...
              {isMoveMode ? (
                <div
                  className="flex flex-col bg-[#F5F5F5] shrink-0"
                  style={{
                    width: "592px",
                    height: "100%",
                    maxHeight: "531px",
                          ...
                          >
                            <div className="flex items-center gap-2 flex-1 min-w-0">
                              {folder.id === "unorganized" ? (
                                <Unlink className="h-4 w-4 text-[#666666] flex-shrink-0" />
                              ) : (
                                <Folder className="h-4 w-4 text-[#666666] flex-shrink-0" />
                              )}
                              <span
                                className="text-sm text-[#171717] truncate"
                                title={folder.name}
                              >
                            ...
                            <div className="flex items-center gap-2">
                              <Badge
                                variant="secondary"
                                className="bg-[#f0f0f0] text-[#171717] text-xs px-2 py-0.5 rounded-full"
                              >
                      ...
                      <div className="space-y-1 pr-2">
                        ...
                        {folders.length === 1 && (
                          ...
                          >
                            ...
                            >
                              Create your first folder and organise your pins
                  ...
                  >
                    <div className="flex" style={{ gap: "10px", opacity: 1 }}>
                      ...
                      >
                        ...
                        >
                          <h3 className="text-xs font-semibold text-[#171717] truncate shrink-0">
                            ...
                            {selectedFolderPins.length > 0 ? (
                              selectedFolderPins
                                ...
                                .map((pin) => {
                                  ...
                                  const isSelected = selectedPinIds.includes(
                                    pin.id
                                  );
                                  return (
                                    <div
                                    ...
                                    >
                                      <Checkbox
                                        checked={isSelected}
                                        onCheckedChange={() =>
                                          handleTogglePinSelection(pin.id)
                                        }
                                        className="shrink-0 cursor-pointer w-[14px] h-[14px] text-white bg-white data-[state=checked]:bg-[#756AF2] border border-[#D4D4D4] data-[state=checked]:border-[#756AF2] rounded-[4px]"
                                      />
                                      <div
                                        style={{
                                          width: "235px",
            ...
            <DialogFooter
              className="flex justify-end items-center bg-white z-20 shrink-0"
              style={{ width: "100%", padding: "10px", gap: "8px", opacity: 1 }}
            >
              {isMoveMode ? (
                <>
                  ...
                  <Button
                    ...
                    style={{
                      ...
                      gap: "8px",
                      opacity: 1,
                      backgroundColor: !selectedMoveFolder
                        ? "rgba(23, 23, 23, 0.3)"
                        : "#171717",
                      flexShrink: 0,
                    ...
                    className={`text-white font-medium text-sm ${
                      selectedMoveFolder ? "hover:bg-[#000000]" : ""
                    }`}
                  >
                    Move Pins
                  </Button>
                ...
                <>
                  ...
                  <Button
                    ...
                    style={{
                      ...
                      flexShrink: 0,
                      cursor:
                        selectedPinIds.length === 0 ? "not-allowed" : "pointer",
                    }}
                    className={`text-white font-medium text-sm ${
                      selectedPinIds.length > 0 ? "hover:bg-[#B91C1C]" : ""
                    }`}
                  >
                    Delete Pins
                  </Button>
                  <Button
                    ...
                    className={`text-white font-medium text-sm ${
                      selectedPinIds.length > 0 ? "hover:bg-[#000000]" : ""
                    }`}
                  >
                    Move Pins
                  </Button>
            ...
            {showDeleteConfirm && (
              ...
              >
                ...
                />
                <div
                  className="relative w-[420px] bg-white rounded-md p-6 shadow-md"
                  style={{ zIndex: 10000 }}
                ...
                >
                  <div className="mb-4">
                    ...
                    <p className="text-sm text-[#666666] mt-2">
                      Are you sure you want to delete {selectedPinIds.length}{" "}
                      pin{selectedPinIds.length > 1 ? "s" : ""}? This action
                      cannot be undone.
                    </p>
                  </div>
                ...
                >
                  ...
                  <div className="flex justify-end gap-2 pt-4">
                    <Button
                      variant="ghost"
                      onClick={() => setIsEditingFolder(false)}
                      className="rounded-lg text-[#1e1e1e] hover:bg-[#f0f0f0] hover:text-[#1e1e1e]"
                    >
                      Cancel
                    </Button>
                    <Button
                      onClick={handleConfirmRenameFolder}
                      className="rounded-lg bg-[#2c2c2c] text-white hover:bg-[#1f1f1f]"
                    >
                      Save
                    </Button>
                  </div>
