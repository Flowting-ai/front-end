...
export default function CompareModelsPage() {
  ...
  useEffect(() => {
    const updateHeight = () => {
      setContainerHeight(Math.min(882, window.innerHeight));
    };
    updateHeight();
        ...
        {[0, 1, 2].map((i) => {
          ...
          return (
            <div key={i} className={styles.modelSlot}>
              {model ? (
                <div className={styles.modelCardSelected}>
                  <div
                  ...
                  <div className={cn(styles.modelInfo, "overflow-hidden")}>
                    <div className={styles.modelName} style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{model.name}</div>
                    <div className={styles.modelMeta} style={{ display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{model.meta}</div>
                  </div>
                  <button
                    className={cn(styles.removeBtn, "absolute top-1/2 -translate-y-1/2 right-2")}
