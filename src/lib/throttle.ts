...
export async function exponentialBackoff<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      // eslint-disable-next-line no-await-in-loop -- retry loop must be sequential; each attempt waits on the previous
      return await fn();
    } catch (error) {
      ...
      if (attempt < maxRetries) {
        ...
        // eslint-disable-next-line no-await-in-loop -- intentional backoff delay between retries
