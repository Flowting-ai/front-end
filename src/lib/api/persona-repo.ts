...
export class PersonaRepoCollection {
  ...
  /** Agents visible inside an org-shared project context. There is only ever
   *  one organization to share to, so this is just the org-shared agents. */
  forTeam(teamId: string | null | undefined): PersonaRepo[] {
    if (!teamId) return this.repos;
    return this.repos.filter(repo => repo.visibility === 'team');
  }
