...
describe('PersonaRepoCollection', () => {
  ...
  const teamAgent = repo({ id: 'team-agent', visibility: 'team', published_version_id: 'v', published_version: version() })
  ...
  it('scopes to a team only via that team deploy set', () => {
    expect(collection.forTeam('team-1', new Set(['team-agent'])).map(r => r.id)).toEqual(['team-agent'])
    // A private agent shared to no team must never leak into a team context.
    expect(collection.forTeam('team-1', new Set(['published']))).toEqual([])
    // Deploy set still loading — scope to nothing, not everything.
    expect(collection.forTeam('team-1', null)).toEqual([])
    // Outside a team the list is unchanged.
    expect(collection.forTeam(null, null)).toHaveLength(3)
  })
})
