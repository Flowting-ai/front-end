...
import {
  CommandCenter,
  type StatusOption,
} from "@/components/personas/command-center";
import {
  WorkflowWrapper,
  type WorkflowItem,
} from "@/components/workflows/workflow-wrapper";
import {
  ChartLine,
  TrendingUp,
  Workflow,
...
export default function WorkflowAdminPage() {
  ...
  const loadWorkflows = React.useCallback(async (silent = false) => {
    ...
    try {
      ...
      const transformedWorkflows: WorkflowItem[] = backendWorkflows.map(
        (wf: WorkflowMetadata, index: number) => ({
          id: wf.id,
          name: wf.name,
                          ...
                          <TableRow className="h-9 rounded-xl border-t border-b border-[#E5E5E5] bg-[#F5F5F5] hover:bg-[#F5F5F5]">
                            <TableHead colSpan={8} className="p-0 h-9">
                              <div className="flex w-fit md:w-full items-center h-full text-[12px] font-inter font-bold uppercase tracking-normal text-[#0E1620]">
                                ...
                                {/* Workflow Unit */}
                                <div className="w-[180px] max-w-45 h-full shrink-0 flex items-center justify-start px-2.5">
                                  <p>Workflow Unit</p>
                                </div>
