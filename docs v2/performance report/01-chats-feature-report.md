...
### Phase 6 — giant-component decomposition (completed, separate session)
The item flagged above as "not started" was later completed in full, step by step with a verification gate (TypeScript + the 271-test suite + a live Playwright smoke test) after every single change. Two files ended up out of scope by deliberate judgment call, not oversight — reasoning below.

**`use-chat-state.ts` (618 → 344 lines).** Moved the ~60% of the file that was pure type declarations (`UIMessage`, `ActivityItem`, `ResponseBlock`, the whole structured-response-block union, etc. — no hook logic) into `src/types/chat.ts`, alongside the `Message`/`Chat`/`Attachment` types they already extend. Pure re-homing, zero behavior change — verified by updating all 24 consumer files' import paths (no re-export shim left behind) and confirming TypeScript stayed clean throughout.

**`ResponseBlocks.tsx` (1808 → 140 lines).** It was really ~15 independent block-renderer components concatenated in one file. Split by exact line-range extraction (a small Node script sliced the original file's verbatim text by verified boundaries, rather than hand-retyping ~1800 lines of JSX/style objects) into: `response-blocks-shared.tsx`, `CitationChip.tsx`, `TextBlockContent.tsx`, and one file each for `AnimatedTable`, `AnimatedBarChart`, `AnimatedSteps`, `AnimatedCodeBlock`, `AnimatedCallout`, `AnimatedTags`, `AnimatedPieChart`, `AnimatedLineChart`, `AnimatedCard`, `AnimatedConnectorError`, `AnimatedSearchTimeout`, `AnimatedFollowUps`. `ResponseBlocks.tsx` itself is now just the `BlockSequenceRenderer` dispatcher. All 7 external consumers (line-renderer.tsx, markdown-utils.tsx, ChatMessage.tsx, the 3 `Xml*.tsx` widgets, the test file) repointed to the new files directly.

**`ChatInterface.tsx` (1483 → ~1360 lines).** Extracted two genuinely self-contained concerns: `useCitationsPanel` (trivial, zero coupling) and `usePinMentions` (the @-mention dropdown, chips, and the `pin:insert` window listener). **Deliberately left the scroll-management cluster in place** (`atBottom`/`atBottomRef`, the virtualizer, 3 scroll-follow effects) — reading the real code showed it's far more entangled with live `messages`/`isStreaming` state and 4 separate call sites (`handleSend`, `sendInitialPrompt`, `handleRegenerate`, `handleEditMessage`) than the initial survey suggested; extracting it would mean either threading everything back through the hook boundary anyway or restructuring `handleSend` itself, for a smaller win than the risk warranted.

**`chat/page.tsx` (1282 → ~1100 lines).** Found the "new chat" pin-mention block was a near-exact duplicate of `ChatInterface.tsx`'s — reused `usePinMentions` (its second consumer), extending the hook with a `pinInsertListenerEnabled` flag to preserve a real behavioral nuance (the two composers briefly coexist mid-`AnimatePresence`-crossfade, and only one may handle a given `pin:insert` event). Also extracted `usePendingPersonaHandoff` (the sessionStorage persona-handoff read, replacing the very hydration-branch bug fixed in Phase 3 — moving it into a hook made it reusable, see below).

**`chats/page.tsx` (961 → 897 lines).** Deleted `formatTaskTimestamp` entirely — it was a near-exact duplicate of the already-imported `formatRelativeTime`, except using plain `new Date(iso)` instead of `formatRelativeTime`'s `parseServerDate`, which specifically corrects backend datetimes serialized without a timezone designator. **A real latent bug killed by deduplication, not just a line-count win.** Merged `ChatsTabDropdown`/`TasksTabDropdown` into one generic `<LibraryTabDropdown<T>>`. Extracted Tasks-mode state into `useTasksLibrary` (its JSX block only needed a small, clean subset of the hook's return — genuine reduced coupling). **Deliberately did not extract Chats-mode the same way** — its JSX is the bulk of the page and would need nearly the entire hook's return threaded straight back in (~20 values); that's relocating coupling, not reducing it.

**Both `ChatInput.tsx` (959 lines) vs `ChatInput/index.tsx` (1357 lines) are live, not dead code.** Traced every consumer: the `chat/` one serves all 6 real chat surfaces; the top-level one is used exactly once, in `agent/configure/layout.tsx`'s "test your agent" panel, wired up with real handlers. Different prop APIs, different design intent (the second looks more design-system-oriented) — unifying them would be a product/architecture decision, not a mechanical split. Left both as-is.

**Bonus fix, discovered as a side effect, not originally in scope:** `project/[id]/chat/[chatId]/page.tsx` turned out to have its own *third* copy of the pin-mention cluster, and — more importantly — still had the exact `typeof window === 'undefined'` lazy-initializer hydration bug already fixed in `chat/page.tsx` back in Phase 3, just never applied here. Generalized `usePendingPersonaHandoff` (parameterized the sessionStorage key, the gating condition, and made the toast optional) so both pages now share one proven fix instead of two near-duplicate ones, and made this file `usePinMentions`'s third consumer. **A second bonus finding surfaced during this fix, left untouched:** this page never forwards `mentionedPins` to `ChatInterface` at all (no `initialMentionedPins` prop) — any pin @-mentioned before the first message in a new project chat is silently dropped. Pre-existing, not introduced by this pass, and out of the scope of what was being fixed here — flagged, not fixed.

**Verification throughout:** every step above passed TypeScript (`tsc --noEmit`, clean every time), the full 271-test suite (271/271 green at every checkpoint), and a live Playwright smoke test specific to whatever moved — including one that required intercepting a network response to find a real project ID, since the project card turned out not to be a plain link. Zero regressions found.
